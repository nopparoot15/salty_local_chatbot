...
if [ "$IS_NIXOS" = true ] && [ -z "${IN_NIX_SHELL:-}" ]; then
    if command -v nix-shell &>/dev/null; then
        ...
        # Always write a fresh shell.nix so stale/broken versions don't block setup
        cat > "$BASE/shell.nix" << 'NIXEOF'
...
"$PIP" install customtkinter Pillow platformdirs deep-translator ddgs \
    --cache-dir "$CACHE" --no-warn-script-location --progress-bar on || {
